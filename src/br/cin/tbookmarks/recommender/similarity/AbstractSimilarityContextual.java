...
abstract class AbstractSimilarityContextual extends AbstractItemSimilarity implements UserSimilarity {
  ...
  /**
   * <p>
   * Creates a possibly weighted {@link AbstractSimilarityContextual}.
   * </p>
   */
  AbstractSimilarityContextual(final DataModel dataModel, Weighting weighting, boolean centerData) throws TasteException {
    super(dataModel);
  ...
  public double userSimilarity(long userID1, long userID2) throws TasteException {
    ...
    ContextualUserPreferenceArray xPrefs = (ContextualUserPreferenceArray)dataModel.getPreferencesFromUser(userID1);
    ContextualUserPreferenceArray yPrefs = (ContextualUserPreferenceArray)dataModel.getPreferencesFromUser(userID2);
    ...
    while (true) {
      ...
      if (hasInferrer || compare == 0) {
        ...
        if (xIndex == yIndex) {
          ...
          xContexts = xPrefs.getContextualPreferences(xPrefIndex);
          yContexts = yPrefs.getContextualPreferences(yPrefIndex);
        } else {
        ...
        sumY2 += y * y;
        double diff = x - y;
        double contextualDiff = compareContexts(xContexts,yContexts);
        sumXYdiff2 += (diff * diff) + (contextualDiff * contextualDiff);
        count++;
      }
      if (compare <= 0) {
  ...
  private double compareContexts(long[] xContexts, long[] yContexts) {
	double numOfDiff = 0;
	
	//double numOfFeatures = xContexts.length; // liberar quando usar outras features//TODO
	double numOfFeatures = 1; //usando apenas tipo do dia (weekday e weekend)
	
	for(int i = 0; i< numOfFeatures;i++){
		if(xContexts[i] != yContexts[i]){
			numOfDiff++;
		}
	}
	return numOfDiff/numOfFeatures;
  }
